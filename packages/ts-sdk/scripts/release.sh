...
cleanup() {
    ...
    if git tag | grep -q "sdk-v$CURRENT_VERSION"; then
        git tag -d "sdk-v$CURRENT_VERSION"
        echo "✓ Removed local git tag sdk-v$CURRENT_VERSION"
    fi
...
if [ "$DRY_RUN" = true ]; then
    ...
    echo "Would create git tag: sdk-v$NEW_VERSION"
